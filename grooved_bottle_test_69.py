print("Cylinders + Curved Polyline slopes!")
...
def bottle():
    ...
    transition_ratio = 0.3   # how tall the slope should be
    ...
    for i in range(num_slices):
        ...
        # --- main cylinder ---
        cyl_h = slice_h
        slice_cyl = Translated(0, 0, z, Cylinder(r0, cyl_h, 0, circle_slices))
        scene += Shape(slice_cyl, material)

        # --- curved polyline slope for groove ---
        if not is_last and r0 != r1:
            num_pts = 10  # smoother curve
            pts = []
            for j in range(num_pts + 1):
                t = j / num_pts
                # cosine easing curve for smooth slope
            ...
            slope_revolved = Revolution(slope_polyline, circle_slices)
